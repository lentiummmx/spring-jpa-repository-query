...
@NamedQueries(
        value = {
            @NamedQuery(
                    name = "NQ_TUTORIALS_PUBLISHED",
                    query = "SELECT t FROM Tutorial t WHERE t.published=:published",
                    hints = {
                        @QueryHint(name = PgsqlQueryHints.HINT_CUSTOM_QUERY_HINT, value = "IndexScan(tutorials idx)")
                    })
        })
@NamedNativeQueries({
    @NamedNativeQuery(
            name = "NNQ_TUTORIALS_PUBLISHED",
    ...
    @NamedNativeQuery(
            name = "Tutorial.findByTitleAndPublished",
